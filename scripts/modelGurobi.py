...
with open(args.initial, 'r') as f:
	def read_2d_array(var, dim_1, dim_2, dtype):
		for i in range(dim_1):
			row = list(map(dtype, f.readline().split()))
			for j in range(dim_2):
				var[i, j].Start = row[j]
	
	read_2d_array(b, len(vehicles), len(requests), int)
	read_2d_array(e, len(vehicles), num_vertices, int)
	read_2d_array(x, len(vehicles), len(edges), int)
	read_2d_array(t, len(vehicles), num_vertices, float)
	read_2d_array(q, len(vehicles), num_vertices, int)

# define objective
m.setObjective(quicksum(t[k_ind, r.destination] for k_ind in range(len(vehicles)) for r_ind, r in enumerate(requests)), GRB.MINIMIZE)
